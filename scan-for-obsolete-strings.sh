...
GITPATH=~/64studio/git/newscoop/
ymlfiles=${GITPATH}newscoop/src/Newscoop/NewscoopBundle/Resources/translations/*.en.yml
...
grep -rq --exclude=*.yml "$strippedline" $GITPATH
...
printf "\nDon't forget to check for false results!\n\n"
 ...
 grep -r --include=*.en.yml --exclude=*.yml "$line" $GITPATH | tee -a matchnotfoundreport
